...
#     x_hat = pos_t_minus1 + np.array(
#         [(delta_sr + delta_sl) / 2 * math.cos(theta_t_minus1 + (delta_sr - delta_sl) / 2 / b)],
...
#     rho_map = data.rhos
#     z_hat_t[0, 1, :] = alpha_map[:] - x_hat[
#         2]  # removing the robots orientation in the world to rotate the line angles into frame
#     z_hat_t[1, 1, :] = rho_map[:] - x_hat[0] * np.cos(alpha_map[:]) + x_hat[1] * np.sin(
#         alpha_map[:])  # translation portion for the lines
#     H_j = np.zeros(2, 3, N)
#
#     H_j[0:1, 0:2, :] = np.array([[0, 0, -1],
#                                  [-np.cos(alpha_map[:]), -np.sin(alpha_map[:]),
#                                   0]])  # it might not be able to handle this notation. Could easily be a loop
#
#     return z_hat_t, H_j
#
#
